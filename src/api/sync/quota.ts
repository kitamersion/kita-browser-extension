...
export const getQuotaUsage = async (): Promise<QuotaInfo | null> => {
  const { data, error } = await getSupabaseClient()
    .from("user_quotas")
    .select<string, { current_bytes: number; quota_tiers: { max_bytes: number } }>("current_bytes, quota_tiers(max_bytes)")
    .maybeSingle();
  ...
  if (!data) return null;
  return { currentBytes: data.current_bytes, maxBytes: data.quota_tiers.max_bytes };
};
